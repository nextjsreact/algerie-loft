...
async function fixCustomersTable() {
  ...
  if (devResponse.status === 404) {
    ...
    console.log('🏗️ Tentative de création de la table...');

    // Get the structure from PROD
    ...
    if (structureResponse.ok) {
      ...
      if (sampleData.length > 0) {
        ...
        // Try to insert the data directly (this will fail if table doesn't exist)
...
CREATE POLICY "Allow all operations for authenticated users" ON public.customers
    FOR ALL USING (auth.role() = 'authenticated');
          ...
          console.log('❌ Erreur lors de l\'insertion:', insertResponse.status);
        }
      }
    }
  } else {
