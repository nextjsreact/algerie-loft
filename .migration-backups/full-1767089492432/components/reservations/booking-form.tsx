...
import { CalendarDays, Users, MapPin, Euro, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
export function BookingForm({ 
  ...
  const loadUserProfileData = async () => {
    try {
      ...
      // For now, we'll use the existing user data and pre-fill what we can
      if (user.full_name) {
        const nameParts = user.full_name.split(' ');
        form.setValue('primaryGuest.firstName', nameParts[0] || '');
        form.setValue('primaryGuest.lastName', nameParts.slice(1).join(' ') || '');
      }
      
      if (user.email) {
        form.setValue('primaryGuest.email', user.email);
      }

      // Load additional profile data if available
      if (user.phone) {
        form.setValue('primaryGuest.phone', user.phone);
      }
      
      if (user.nationality) {
        form.setValue('primaryGuest.nationality', user.nationality);
      }
      
      if (user.date_of_birth) {
        form.setValue('primaryGuest.dateOfBirth', user.date_of_birth);
      }

      // Load preferences if available
      if (user.preferences?.bookingDefaults) {
        const defaults = user.preferences.bookingDefaults;
        if (defaults.arrivalTime) {
          form.setValue('preferences.arrivalTime', defaults.arrivalTime);
        }
        if (defaults.specialRequests) {
          form.setValue('preferences.specialRequests', defaults.specialRequests);
        }
        if (defaults.accessibilityNeeds) {
          form.setValue('preferences.accessibilityNeeds', defaults.accessibilityNeeds);
        }
        if (defaults.dietaryRestrictions) {
          form.setValue('preferences.dietaryRestrictions', defaults.dietaryRestrictions);
        }
      }
    } catch (error) {
...
// Step Components
function GuestInfoStep({ form, t }: { form: any; t: any }) {
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-4">{t('steps.guestInfo.title')}</h3>
        <p className="text-muted-foreground mb-6">{t('steps.guestInfo.description')}</p>
      </div>
          ...
          {formData.additionalGuests && formData.additionalGuests.length > 0 && (
            <div>
              ...
              {formData.additionalGuests.map((guest: AdditionalGuest, index: number) => (
                <p key={index} className="text-sm">{guest.firstName} {guest.lastName} ({guest.age} years old)</p>
              ))}
            </div>
          )}
        </CardContent>
