...
export function PropertyInquiryForm({ 
  ...
  const form = useForm<PropertyInquiryData>({
  ...
  const handleSubmit = async (data: PropertyInquiryData) => {
    ...
    <Card className={className}>
      {showTitle && (
        <CardHeader>
          ...
          <CardDescription>
            {propertyName ? t("form.descriptionWithProperty", { property: propertyName }) : t("form.description")}
          </CardDescription>
        </CardHeader>
      )}
      <CardContent>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
          ...
          {propertyName && (
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              <h4 className="font-medium text-blue-900 mb-2">{t("form.propertyInfo")}</h4>
              <p className="text-blue-800">{propertyName}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="viewing">{t("form.inquiryTypes.viewing")}</SelectItem>
                  <SelectItem value="information">{t("form.inquiryTypes.information")}</SelectItem>
                  <SelectItem value="rental">{t("form.inquiryTypes.rental")}</SelectItem>
                  <SelectItem value="purchase">{t("form.inquiryTypes.purchase")}</SelectItem>
                  <SelectItem value="management">{t("form.inquiryTypes.management")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="preferredContactTime">{t("form.preferredContactTime")}</Label>
              <Select
                value={form.watch("preferredContactTime")}
                onValueChange={(value) => form.setValue("preferredContactTime", value as any)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="morning">{t("form.contactTimes.morning")}</SelectItem>
                  <SelectItem value="afternoon">{t("form.contactTimes.afternoon")}</SelectItem>
                  <SelectItem value="evening">{t("form.contactTimes.evening")}</SelectItem>
                  <SelectItem value="anytime">{t("form.contactTimes.anytime")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="moveInDate">{t("form.moveInDate")}</Label>
            <Input
              id="moveInDate"
              type="date"
