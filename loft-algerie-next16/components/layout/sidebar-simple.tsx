...
export function SimpleSidebar({ user, unreadCount, className }: SidebarProps) {
  ...
  const getRoleDisplayName = (role: string) => {
    switch (role) {
      ...
      case 'manager': return tRoles('manager')
      case 'member': return tRoles('member')
      case 'executive': return tRoles('executive')
      default: return role
    }
  }

  return (
    <div className={cn("flex h-full w-72 flex-col bg-gray-900 dark:bg-gray-900", className)} data-testid="sidebar" dir="auto">
              ...
              >
                <CollapsibleTrigger className={cn(
                  ...
                  <div className="flex items-center">
                    <item.icon className="mr-3 rtl:mr-0 rtl:ml-3 h-5 w-5 flex-shrink-0" />
                    {item.name}
                  </div>
                  {isSettingsOpen ? (
                ...
                <CollapsibleContent className="pl-8 space-y-1">
                  {item.subItems.map((subItem) => (
                    ...
                    >
                      <subItem.icon className="mr-3 rtl:mr-0 rtl:ml-3 h-5 w-5 flex-shrink-0" />
                      {subItem.name}
                    </Link>
                  ))}
                </CollapsibleContent>
          ...
          return (
            ...
            >
              ...
              {item.href === "/conversations" && unreadMessagesCount > 0 && (
                <NotificationBadge count={unreadMessagesCount} className="ml-2 rtl:ml-0 rtl:mr-2" />
              )}
              {item.href === "/notifications" && realtimeUnreadCount > 0 && (
                <NotificationBadge count={realtimeUnreadCount} className="ml-2 rtl:ml-0 rtl:mr-2 animate-pulse" />
              )}
            </Link>
          )
        })}
        ...
        <div className="flex items-center gap-3 p-2 rounded-lg bg-gray-800/50 hover:bg-gray-800 transition-colors">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-white truncate">
              {user.full_name === 'member1' ? t('member1') : user.full_name}
            </p>
            <p className="text-xs text-gray-300 capitalize">{getRoleDisplayName(user.role)}</p>
          </div>
        </div>
        <Button
          variant="ghost"
