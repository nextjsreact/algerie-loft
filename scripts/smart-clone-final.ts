#!/usr/bin/env tsx
/**
 * CLONAGE INTELLIGENT FINAL PRODUCTION → DÉVELOPPEMENT
 * ====================================================
 * 
 * Ce script gère automatiquement les différences de schéma détectées :
 * - currencies: decimal_digits manquant dans DEV
 * - categories: updated_at manquant dans DEV
 * - lofts: price_per_month requis dans DEV
 */
...
class SmartCloneFinal {
  ...
  private readonly COLUMN_ADAPTATIONS: { [table: string]: { [column: string]: any } } = {
    currencies: {
      // Colonnes à ignorer (présentes dans PROD mais pas dans DEV)
    ...
    lofts: {
      // Valeurs par défaut pour colonnes requises dans DEV
  ...
  public async cloneAllTables(): Promise<void> {
    console.log('🚀 CLONAGE INTELLIGENT FINAL PRODUCTION → DÉVELOPPEMENT')
  ...
  private printSummary(totalRecords: number, startTime: number): void {
    ...
    console.log('• Différences de schéma gérées intelligemment')
  }
  ...
  public async clone(): Promise<void> {
    try {
      ...
      // 2. Clonage intelligent de toutes les tables
...
// Exécution du script
async function main() {
  const cloner = new SmartCloneFinal()
  await cloner.clone()
}
