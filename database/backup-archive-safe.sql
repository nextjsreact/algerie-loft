-- =====================================================
-- BACKUP AND ARCHIVE SYSTEM - SAFE VERSION
-- =====================================================
-- This script creates tables and handles system_configurations properly
...
-- =====================================================
-- INSERT DEFAULT CONFIGURATION SAFELY
-- =====================================================

-- Insert backup configuration with proper handling of system_configurations structure
DO $$
DECLARE
    has_data_type BOOLEAN;
    has_description BOOLEAN;
    has_is_sensitive BOOLEAN;
BEGIN
    ...
    ) INTO has_is_sensitive;

    -- Insert configurations based on table structure
    IF has_data_type AND has_description AND has_is_sensitive THEN
        -- Full structure with all columns
        INSERT INTO system_configurations (category, config_key, config_value, data_type, description, is_sensitive, modified_at)
        VALUES 
            ('backup', 'retention_days', '30', 'integer', 'Number of days to retain backups', false, NOW()),
            ('backup', 'compression_enabled', 'true', 'boolean', 'Enable backup compression', false, NOW()),
            ('backup', 'encryption_enabled', 'true', 'boolean', 'Enable backup encryption', false, NOW()),
            ('backup', 'max_concurrent_backups', '2', 'integer', 'Maximum concurrent backup operations', false, NOW()),
            ('backup', 'storage_location', '"/backups"', 'string', 'Backup storage location', false, NOW()),
            ('backup', 'backup_schedule', '{"full_backup_cron": "0 2 * * 0", "incremental_backup_cron": "0 2 * * 1-6"}', 'json', 'Backup schedule configuration', false, NOW()),
            ('backup', 'notification_settings', '{"on_success": true, "on_failure": true, "email_recipients": []}', 'json', 'Backup notification settings', false, NOW())
        ON CONFLICT (category, config_key) DO NOTHING;
    ELSIF has_data_type AND has_description THEN
        -- Structure with data_type and description but no is_sensitive
        INSERT INTO system_configurations (category, config_key, config_value, data_type, description, modified_at)
        VALUES 
            ('backup', 'retention_days', '30', 'integer', 'Number of days to retain backups', NOW()),
            ('backup', 'compression_enabled', 'true', 'boolean', 'Enable backup compression', NOW()),
            ('backup', 'encryption_enabled', 'true', 'boolean', 'Enable backup encryption', NOW()),
            ('backup', 'max_concurrent_backups', '2', 'integer', 'Maximum concurrent backup operations', NOW()),
            ('backup', 'storage_location', '"/backups"', 'string', 'Backup storage location', NOW()),
            ('backup', 'backup_schedule', '{"full_backup_cron": "0 2 * * 0", "incremental_backup_cron": "0 2 * * 1-6"}', 'json', 'Backup schedule configuration', NOW()),
            ('backup', 'notification_settings', '{"on_success": true, "on_failure": true, "email_recipients": []}', 'json', 'Backup notification settings', NOW())
        ON CONFLICT (category, config_key) DO NOTHING;
    ELSIF has_data_type THEN
        -- Structure with data_type only
        INSERT INTO system_configurations (category, config_key, config_value, data_type, modified_at)
        VALUES 
            ('backup', 'retention_days', '30', 'integer', NOW()),
            ('backup', 'compression_enabled', 'true', 'boolean', NOW()),
            ('backup', 'encryption_enabled', 'true', 'boolean', NOW()),
            ('backup', 'max_concurrent_backups', '2', 'integer', NOW()),
            ('backup', 'storage_location', '"/backups"', 'string', NOW()),
            ('backup', 'backup_schedule', '{"full_backup_cron": "0 2 * * 0", "incremental_backup_cron": "0 2 * * 1-6"}', 'json', NOW()),
            ('backup', 'notification_settings', '{"on_success": true, "on_failure": true, "email_recipients": []}', 'json', NOW())
        ON CONFLICT (category, config_key) DO NOTHING;
    ELSE
        -- Basic structure without data_type
