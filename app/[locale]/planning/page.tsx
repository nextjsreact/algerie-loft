    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.sent === data.total) {
          toast.success(`✅ Planning envoyé avec succès à ${data.sent} agent${data.sent > 1 ? 's' : ''}`)
        } else {
          toast.success(`Planning envoyé à ${data.sent}/${data.total} agents`)
          failed.forEach((r: any) => toast.warning(`⚠️ ${r.agent} : ${r.error}`))
        }
        setSent(true)
      } else {
      ...
      <div className="flex items-start justify-between gap-1">
        <div className="flex-1 min-w-0 space-y-0.5">
          ...
          <p className={`font-semibold text-sm ${textColor}`}>
            ...
            {isAirbnb && <span className="ml-1.5 inline-flex items-center rounded-full bg-blue-100 px-1.5 py-0.5 text-[10px] font-semibold text-blue-700">🔵 Airbnb</span>}
          </p>
